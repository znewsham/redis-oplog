...
export default function(cursorDescription, ordered, callbacks) {
    ...
    Meteor._noYieldsAllowed(function() {
        ...
        if (_.has(self._observeMultiplexers, observeKey)) {
            ...
            const od = multiplexer._observeDriver;
            const multiSuppress = (od.options && od.options._suppress_initial) || false;
            const callSuppress = callbacks._suppress_initial || false;
            if (multiSuppress !== callSuppress) {
              if (!od.options) {
                od.options = {};
              }
              od.options._suppress_initial = callSuppress;
              if (!od.options._suppress_initial) {
                od.observableCollection.init();
              }
            }
        } else {
